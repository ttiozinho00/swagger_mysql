...
const trans = require('../plugins/transaction');
...
router.get('/', function(req, res, next) {
	...
	if (vm.profile_id)
	{
		and.push(sequelize.where(sequelize.col('profile_id'), parseInt(vm.profile_id)));
	}

	if (vm.name)
	{
		and.push(sequelize.where(sequelize.fn('lower', sequelize.col('name')), {[sequelize.Op.like]: `%${vm.name.toLowerCase()}%`}));
	}
	
	if (vm.is_active != null)
	{
		and.push(sequelize.where(sequelize.col('is_active'),vm.is_active == 'true' ? 1 : 0));
	}
